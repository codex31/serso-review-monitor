...
export function useAuth(options?: UseAuthOptions) {
  ...
  useEffect(() => {
    if (!redirectOnUnauthenticated || meQuery.isLoading || logoutMutation.isPending || state.user || !redirectPath) return;
    if (typeof window !== "undefined" && window.location.pathname !== redirectPath) window.location.href = redirectPath;
